...
var Library = {
  ...
  Generate: function(params) {
    ...
    function getChoiceData(que) {
      ...
      var answerIndex = getAnswerIndex(que);
      var copyData = data.slice(50, 150);
      var newData = [];
  ...
  drawChoice: function(params) {
    ...
    if (type === 0) {
      ...
      paper.rect(30, 225, 300, 330).attr({
        'fill': '#ffe6cc'
      });
      ...
      for (var k = 0; k < exampleInfo.choices.length; k++) {
        ...
        choices[k].circle(110 + (k % 2) * 140, 320 + Math.floor(k / 2) * 140, 55).attr({
          'fill': 'white'
        });

        choices[k].text(110 + (k % 2) * 140, 340 + Math.floor(k / 2) * 140, exampleInfo.choices[k]).attr({
          'font-size': 55,
          'fill': '#777777',
          'text-anchor': 'middle'
        });
  ...
  choiceCheck: function(params) {
    ...
    function handler() {
      ...
      var fdEl;
      if (check) {
        fdEl = Paper.circle(180, 388, 135).attr({
          'stroke': 'tomato',
          'stroke-width': 35,
          'fill': 'none',
          'opacity': 0.5
        ...
        var fdEl1 = Paper.line(55, 255, 304, 522).attr({
          'stroke': '#bbbbbb',
          'stroke-width': 30,
          'stroke-linecap': 'round',
          'opacity': 0.5
        });
        var fdEl2 = Paper.line(304, 255, 55, 522).attr({
          'stroke': '#bbbbbb',
          'stroke-width': 30,
          'stroke-linecap': 'round',
          'opacity': 0.5
